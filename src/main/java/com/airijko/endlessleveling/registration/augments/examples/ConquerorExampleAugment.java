...
/**
 * Example augment with Java-backed behavior: stacking on-hit damage with true damage at max stacks.
 * This demonstrates how to create a custom augment factory beyond pure data-only definitions.
 */
public final class ConquerorExampleAugment extends Augment implements AugmentHooks.OnHitAugment {
    ...
    public ConquerorExampleAugment(AugmentDefinition definition) {
        ...
        this.maxStackFlatTrueDamage = Math.max(0.0D,
                AugmentValueReader.getDouble(bonusTrueDamage, "value", 0.0D));
        ...
        this.stackDurationMillis = AugmentUtils.secondsToMillis(
                AugmentValueReader.getDouble(duration, "seconds", 0.0D));
    }
    ...
    public float onHit(AugmentHooks.HitContext context) {
        ...
        if (stackDurationMillis > 0L && state.getStacks() > 0
                && state.getExpiresAt() > 0L && now >= state.getExpiresAt()) {
            state.clear();
        }
        ...
        float updatedDamage = AugmentUtils.applyAdditiveBonusFromBase(
                context.getDamage(), context.getBaseDamage(), bonusMultiplier);
